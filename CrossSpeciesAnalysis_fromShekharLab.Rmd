...
```{r}
RGC_types <- readRDS("F:/Lab/sanes data/Multiple Species RGCs/RGC_types_nolamp_v6.rds")
...
DimPlot(RGC_types, group.by = "NOG", label = TRUE)
...
FeaturePlot(RGC_types, reduction = 'umap',features='Robo1')
dev.off()
```
...
```{r}
RGC_types <- readRDS("F:/Lab/sanes data/Multiple Species RGCs/RGC_types_nolamp_v6.rds")
RGC_types <- readRDS("F:/Lab/sanes data/Multiple Species RGCs/RGC_types_mammals_v6.rds")
...
# 21 NOGS
plot_markers <- c("Esrrg", "Ppargc1a3", "Nedd49", "Syt6", "Ppp1r1c", "Nedd410", "Syt6", "Syt6", "Syt6", "Syt6", "PLXNA2", "SGCD", "RUNX1", "NFIB", "PCDH17", "SORCS3", "CDH11", "CDH6", "ROBO1", "TMEM132C" , "CHST9")

# 21 NOGS
plot_markers <- c("RXRG", "IRX3", "GULP1", "IL1RAPL2", "PROX1", "NDST4", "MDFIC", "IRX6", "TBX20", "GRIK3", "PLXNA2", "SGCD", "RUNX1", "NFIB", "PCDH17", "SORCS3", "CDH11", "CDH6", "ROBO1", "TMEM132C" , "CHST9")

dsgc_markers <- c('SYT6','NPY','PAPPA','ROBO1','FIBCD1','TBX5','PTPRK')

plot_markers <- c("Irx3")

RGC_types@meta.data$NOG <- factor(RGC_types@meta.data$NOG, levels = 1:4)

FeaturePlot(RGC_types, features = 'ROBO1', reduction = "umap", cols = c('white','red'),min.cutoff = 0)
...
DotPlot(RGC_types, features = dsgc_markers, assay = "RNA", group.by = "NOG", cols = c("white", "#df2323")) + RotatedAxis()
dev.off()

new_dotplot(RGC_types, genes.use =  head(subset(markers, cluster == "19"), 10)$gene, ident.use = "NOG", 
...
new_dotplot(RGC_types, genes.use = plot_markers, ident.use = "NOG", col.low = "white", col.high =  "#6e78ff", species.threshold = .3, breaks = c(0,4,8,12))

pdf("../Figures/Fig 4/4d_mammals_spec_exp.pdf", w=9, h=5, useDingbats = FALSE)
p
dev.off()

plot_markers <- head(subset(markers, cluster == "19"), 21)$gene


new_dotplot(RGC_types, genes.use = head(subset(markers, cluster == "19"), 21)$gene, ident.use = "NOG", col.low = "white", col.high =  "#df2323", species.threshold = .3, breaks = c(0,4,8,12)) + RotatedAxis()
